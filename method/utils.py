...
from imageQuality.watson_vgg import WatsonDistanceVgg
...
def gen_pgd_confs(eps, alpha, iter, input_range=(-1, 1)):
    conf = {}
    conf['eps'] = eps / 255.0 * (input_range[1] - input_range[0])   # 对抗扰动的大小范围
    conf['alpha'] = alpha / 255.0 * (input_range[1] - input_range[0])   # 对抗样本更新的步长大小
    conf['iter'] = iter # 攻击的迭代次数
    return conf
...
def get_img_tensor(img_path, device):
    img_tensor = pil_to_tensor(Image.open(img_path).convert("RGB").resize((256, 256))) / 255
...
def compute_lpips(image, org_image, device=None):
    loss_percep = WatsonDistanceVgg(reduction='sum')
    loss_percep.load_state_dict(torch.load('./imageQuality/rgb_watson_vgg_trial0.pth', map_location='cpu'))
    loss_percep = loss_percep.to(device)
    lpips = loss_percep((1+image)/2.0, (1+org_image)/2.0)/ org_image.shape[0]
    
    return lpips


def preprocess(image, res: int = 256):
    """
        pil to tensor
        output: (1, 3, 256, 256)
        output_range: (0, 1)
...
class CustomImageFolder(Dataset):
    def __init__(self, data_dir, data_cnt=-1, is_transform=False):
        self.data_dir = data_dir
        self.filenames = glob.glob(os.path.join(data_dir, "*"))
        self.filenames = natsorted(self.filenames, alg=ns.PATH)

        if data_cnt != -1:
            self.filenames = self.filenames[:data_cnt]
        if data_dir[-1] != '/':
            data_dir += '/'
        self.img_ids = [x.replace(data_dir, '').split('.')[0] for x in self.filenames]
    ...
    def __getitem__(self, idx):
        ...
        if self.is_transform:
            transform = transforms.Compose([
                transforms.Resize((512, 512)),  # (256, 256)
...
def get_imagenet_label_id(path=None):
    with open(path, 'r') as f:
        label_id = []
        for i in f.readlines():
            label_id.append(int(i.rstrip()) - 1)   # The label number of the imagenet-compatible dataset starts from 1.
...
def get_imagenet_label_name(label_id):
    id = int(label_id)
    label_name = Label[id].split(",")[0]
    return label_name


def get_diffusiondb_prompt(path=None):
    with open(path, 'r', encoding='utf-8') as file:
        next(file)  # 跳过表头行
        prompts = []
        for line in file:
            parts = line.strip().split(' ', 1)
            imageName, prompt = parts
